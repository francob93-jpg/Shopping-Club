...
function Dashboard() {
  const stats = [
    {
      lbl: 'Usuarios activos', val: '2.847', delta: '+12.4%', dir: 'up',
    ...
    {
      lbl: 'Canjes del mes', val: '1.412', delta: '+8.1%', dir: 'up',
      hint: '47 hoy',
      ico: <Icon.Receipt/>,
    ...
    {
      lbl: 'Giros de ruleta', val: '987', delta: '−2.3%', dir: 'down',
      hint: '34 hoy',
      ico: <Icon.Wheel/>,
    ...
    {
      lbl: 'Beneficios activos', val: '24', delta: '+3', dir: 'up',
      hint: 'de 31 totales',
      ico: <Icon.Tag/>,
  ...
  // Bar chart data — canjes por día (últimos 30)
  const canjesDaily = [22,28,31,26,34,42,38,29,33,37,41,45,39,48,52,44,38,42,49,55,47,51,58,53,46,50,57,62,58,64];
  ...
  const max = Math.max(...canjesDaily);

  return (
    <div className="ad-content">
      <PageHead title="Dashboard" sub="Hola Marina · Resumen de actividad del club">
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:14, marginBottom:18}}>
        {stats.map((s, i) => (
          <div key={i} className="ad-stat">
            <div className="lbl">
              <span className="lbl-ico" style={{color: s.color}}>{s.ico}</span>
              {s.lbl}
            </div>
            <div className="val">{s.val}</div>
            <div style={{display:'flex', alignItems:'center', gap:8, marginTop:10}}>
              <span className={`delta ${s.dir}`}>
                {s.dir === 'up' ? <Icon.TrendUp/> : <Icon.TrendDown/>}
                {s.delta}
              </span>
        ...
        <div className="ad-card">
          ...
          <div style={{padding:'22px 18px 20px'}}>
            <div style={{display:'flex', alignItems:'flex-end', gap:4, height:200, position:'relative'}}>
              ...
              {canjesDaily.map((v, i) => {
                const h = (v / max) * 100;
                const isToday = i === canjesDaily.length - 1;
                return (
            ...
            {topBenefits.map(b => (
              <div key={b.rank} style={{display:'flex', alignItems:'center', gap:10, padding:'10px 12px'}}>
                ...
                <div style={{flex:1, minWidth:0}}>
                  <div style={{fontSize:13, fontWeight:600, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{b.t}</div>
                ...
                <div style={{textAlign:'right', flexShrink:0}}>
                  <div className="mono" style={{fontSize:13, fontWeight:600}}>{b.n}</div>
                  <div style={{fontSize:10, color:'var(--ad-muted)', textTransform:'uppercase', letterSpacing:'0.05em', fontWeight:600}}>canjes</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* bottom row: recent + wheel summary */}
      <div style={{display:'grid', gridTemplateColumns:'1.6fr 1fr', gap:14}}>
        <div className="ad-card">
          <div className="ad-card-head">
            ...
            <button className="ad-btn subtle sm"><Icon.Refresh/> Actualizar</button>
          </div>
            ...
            <tbody>
              {recentRedemptions.map((r, i) => (
                <tr key={i}>
                  ...
                  <td>{r.ben}</td>
                  <td><span className="mono cell-muted" style={{fontSize:12}}>{r.cred}</span></td>
                  <td style={{textAlign:'right'}} className="cell-muted">{r.ago}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="ad-card">
          ...
          <div style={{padding:'18px'}}>
            <div style={{display:'flex', alignItems:'center', gap:14, padding:'12px 14px', background:'var(--ad-surface-2)', borderRadius:10, border:'1px solid var(--ad-line)'}}>
              <div style={{
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:13.5, fontWeight:700}}>Café gratis</div>
                <div style={{fontSize:11.5, color:'var(--ad-muted)', marginTop:2}}>Entregado 412 veces · 41.7%</div>
              </div>
            </div>

            <div style={{marginTop:14}}>
              <div style={{fontSize:12, fontWeight:600, color:'var(--ad-muted)', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:10}}>Distribución</div>
              {[
                ...
                <div key={i} style={{display:'flex', alignItems:'center', gap:10, marginBottom:8}}>
                  ...
                  <span className="mono" style={{fontSize:12, color:'var(--ad-muted)'}}>{s.n}</span>
                </div>
              ))}
            </div>
          </div>
